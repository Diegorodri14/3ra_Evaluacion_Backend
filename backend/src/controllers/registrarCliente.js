...
registerControllerCliente.register = async (req, res) => {
  try {
    const { name, email, password, isVerified, loginAttemps, timeOut } = req.body;

    const clienteExistente = await modelCliente.findOne({ email });
...
registerControllerCliente.verifyCode = async (req, res) => {
  try {
    ...
    const nuevoCliente = modelCliente({
      name,
      email,
      password,
      isVerified: true,
...
export default registerControllerCliente;